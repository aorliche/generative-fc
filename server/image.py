...
def histogram(ys, ylabels=None, bins=20, xlabel=''):
    fig, ax = plt.subplots()
    if ylabels is None:
...
def imshow_private(fc, colorbar=True, reverse_cmap=False, bounds=None):
    fig, ax = plt.subplots()
    ...
    if bounds is not None:
        for i,a in enumerate(bounds[0:-1]):
            loc = (bounds[i]+bounds[i+1])/2-3
            ax.text(loc, 10, i, color='black')
            ax.text(5, loc+6, i, color='black')
...
def groups_hist_private(df, groups, field):
    ...
    for group in groups.keys():
        ...
        ys.append(lst)
    return histogram(ys, ylabels, xlabel=field)
...
def imshow(fc, colorbar=True, reverse_cmap=False, bounds=None):
    return mp_wrap(imshow_private, fc, colorbar, reverse_cmap, bounds)
